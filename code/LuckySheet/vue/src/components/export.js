...
export var exportExcel = function(luckysheet, value) {
  ...
  luckysheet.forEach(function(table) {
    ...
    const rowData = table.config.rowlen;

    // 3.设置单元格合并,设置单元格边框,设置单元格样式,设置值
    setStyleAndValue(table.data, worksheet)
    setMerge(merge, worksheet)
    setBorder(borderInfo, worksheet)
    setRowColSize(columnData,rowData, worksheet);
...
var setRowColSize = function(columnData,rowData, worksheet){
  ...
  if(rowData){
    _.each(rowData, (v,k)=>{
      let rowNum = parseInt(k)+1;
      let temp = worksheet.getRow(rowNum);
      temp.height = v;
    })
  }
}

var setImage = function(luckyImage, worksheet) {
  // console.log('luckyImage',luckyImage);
  luckyImage.forEach(function(img){
    console.log('img',img);
